...
CONFIG = os.path.join(os.path.dirname(__file__), '../', 'resources/ipfconfig.cnfg')
...
class Main(object):
    ...
    def __get_subjects(self):
        ...
        for subject in range(len(self._cfg["subjects"])):
            subject_ID = self._cfg["subjects"][subject]
            path_to_subject = os.path.join(init_path, "IPF"+self._cfg["subjects"][subject])
            for date in range(len(self._cfg["subject_data_"+str(self._cfg["subjects"][subject])])):
                path_to_sub_date = os.path.join(path_to_subject,
                                            self._cfg["subject_data_"+subject_ID][date],
                                            self._cfg["protocol"],
                                            self._cfg["path"])

                path_to_file = os.path.join(path_to_sub_date, self._cfg["lung"] + "_fitted.mesh")
    ...
    def __align(self, scaling=False):
        ...
        for mesh in meshes:
            d, Z, tform, m, aligned_mesh_path = self._mesh.align_mesh(reference_mesh, mesh, scaling=scaling,
                                                                      reflection='best')
    ...
    def __get_score(self):
        ...
        m_distance = list()
        score_array = np.chararray((len(subject_names), self._sfeal.get_number_of_modes() + 1), itemsize=25,
                                   unicode=True)
